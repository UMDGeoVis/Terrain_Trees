...
template<class T> void morse_simplification(T& tree, cli_parameters &cli, coord_type& mode_to_correct, double& length_limit);
template<class T> void load_terrain(T& tree, cli_parameters &cli);
template<class T> void extract_features(T& tree, cli_parameters &cli, Forman_Gradient_Simplifier& forman_simplifier, Forman_Gradient& forman_gradient, string file_name, double& length_limit);
...
int main(int argc, char** argv )
{
    ...
    morse_simplification(ptree,cli, mode_to_correct, length_limit);    
...
template<class T> void morse_simplification(T& tree, cli_parameters &cli, coord_type& mode_to_correct, double& length_limit)
{
    ...
    /// ---- MORPHOLOGICAL SIMPLIFICATION --- ///        
    {
       ...
       // forman_simplifier.get_incidence_graph().print_stats(true);
        ...
        extract_features(tree, cli, forman_simplifier, forman_gradient, "after", length_limit);
    }
}
